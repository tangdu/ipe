...
/**
 * Created with IntelliJ IDEA. User: tangdu Date: 13-7-28 Time: 上午11:49 To
	...
	public static void main(String args[]) {
		...
		try {
			...
			for (String entity : entitys) {
				...
				model.put("contrlPackName",
						"com.ipe.module.core.web.controller");
				...
				FileUtils.write(
						new File(dao_path + "\\" + entity + "Dao.java"),
				...
				template.process(model, result);
				FileUtils.write(new File(service_path + "\\" + entity
						+ "Service.java"), result.toString(), "UTF-8");

				// constroller templdate
				result = new StringWriter();
				template = cfg.getTemplate("Controller.ftl");
				template.process(model, result);
				FileUtils.write(new File(controller_path + "\\" + entity
						+ "Controller.java"), result.toString(), "UTF-8");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
