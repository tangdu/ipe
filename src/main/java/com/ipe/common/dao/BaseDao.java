...
public abstract class BaseDao<M extends Serializable, PK extends Serializable> {
    ...
    public void batchDel(PK ids[]) {
        String hql = "delete from " + HQL_TABNEME + " where id= ?";
        execute(hql, ids);
    }
    ...
    @SuppressWarnings("unchecked")
    public <T> List<T> bulid(String hql, PageModel model, final Map<String, Object> map) {
	...
	public <T> List<T> sqllist(String sql){
    	return (List<T>)this.getSession().createSQLQuery(sql).addEntity(entityClass).list();
    }
    @SuppressWarnings("unchecked")
	public <T> T  sqlFindOne(String sql){
    	List<T> ll=(List<T>)this.getSession().createSQLQuery(sql).addEntity(entityClass).list();
